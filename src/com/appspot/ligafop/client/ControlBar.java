...
/**
 * This is a UI component that corresponds to the bar at the top of the Application with the logo
 * and some controls.
 */
public class ControlBar extends Widget {
	...
	public static class Controls {
		...
		/**
		 * Creates a control styled by a specified CSS class selector name. Performs the inputed
		 * action on click.
		 * 
		 * @param cssClassName
		 * @param action
		...
		public void addControl(String cssClassName, EventListener action) {
			...
			control.setClassName(resources.controlBarCss().control() + " " + cssClassName);
		...
		public void showControls() {
			for (int i = 0, n = controls.size(); i < n; i++) {
				DivElement divElement = controls.get(i);
				divElement.getStyle().setProperty("display", "");
			}
		}
		...
		public void hideControls(boolean isGerente) {
			if (!isGerente) {
				for (int i = 0, n = controls.size(); i < n; i++) {
					DivElement divElement = controls.get(i);
					String className = divElement.getClassName();
					if (className.contains("plusJogo") || className.contains("plusPartida")) {
						divElement.getStyle().setProperty("display", "none");
					}
				}
			}
		}
	 ...
	 * 
	 * @param parentElement
	 *            the DOM element we want to attach to
	 * @param controls
	 *            the controls we want to include
	 */
	...
	public void disableControls(boolean isGerente) {
		controls.hideControls(isGerente);
